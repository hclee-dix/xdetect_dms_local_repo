...
from dev.ai.xSmoking.net.keypoint_preprocess import EvalAffine, TopDownEvalAffine,get_affine_transform
...
def decordImageFromBase64(im_file,im_info):
    if isinstance(im_file,str):
        buf = base64.b64decode(im_file)
...
def decordImageFromBuffer(im_buf, im_info):
    if isinstance(im_buf,bytes):
        img = np.frombuffer(im_buf,dtype='uint8')
        img = cv2.imdecode(img,1)
        im = cv2.cvtColor(img,cv2.COLOR_BGR2RGB)
    else:
        im = im_buf
...
def decordVideoFromBuffer(im_buf):
    video_stream = io.BytesIO(im_buf)
    temp_file_path = createTempFile(video_stream.getvalue())
    return temp_file_path

def createTempFile(src:Optional[Any] = None, write=True,suffix=''):
    temp_file_path = ''
    with tempfile.NamedTemporaryFile(delete=False,suffix=suffix) as fp:
        if src and write: fp.write(src)
        temp_file_path = fp.name
    return temp_file_path

def deleteTempFilePath(*pathList):
    for path in pathList:
        os.remove(path)

def encordImageToBase64(img):
    ret, buf = cv2.imencode('.jpg',img)
    im_file = base64.b64encode(buf)
    return im_file

def encordFileToBase64(path):
...
class ShortSizeScale(object):
    ...
    def __call__(self, img):
        ...
        if self.backend == 'pillow':
            result_img = img.resize((ow, oh), Image.Resampling.BILINEAR)
...
def preprocess(im, preprocess_ops):
    ...
    im, im_info = decordImageFromBase64(im, im_info)
