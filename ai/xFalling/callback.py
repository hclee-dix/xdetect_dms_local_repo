...
#
from dev.util.ops import getFallingResultFromSingle,getFallingResultFromTotal
#

class XFallingDetectorCallback(XCallback):
    ...
    def onPredictEnd(self,imageList,perfList,resultList):
        ...
        ##
        result,accuracy,perf = getFallingResultFromTotal(perfList,resultList)
        history = IDetectHistory(
            ...
            result_list=[IDetectHistoryResult(index=int(data[3,1]),conf=data[3,2],name=self.labelMap[int(data[3,1])]) for data in result]
        )
        print(history)
    ...
    def onPredictEpochEnd(self,image,perf,result,frame_num=None):
        ...
        storage_url = uploadFile(src_path,dst_path)
        totalResult,accuracy = getFallingResultFromSingle(result)
        history = IDetectHistory(
            org_id=self.request.organization_id,
            project_id=self.request.project_id,
            d_detect_model_id=self.request.d_detect_model_id,
            ...
            fps=1/perf,
            accuracy=float(accuracy),
            result_list=[IDetectHistoryResult(index=int(data[1]),conf=data[2],name=self.labelMap[int(data[1])]) for data in totalResult]
        )
        print(history)
