...
from dev.ai.xFalling.net.utils import argsparser, Timer, get_current_memory_mb
from dev.ai.xFalling.net.detector import Detector
from attr_infer import AttrDetector
from keypoint_buffer import KeyPointSequence,KeyPointBuff
...
class DetActionRecognizer(object):
    ...
    def postprocess(self, det_result, mot_result):
        np_boxes_num = det_result['boxes_num'] # [N,] => [100,...]
        #boxes [N*100,6] => [cls,score,xmin,ymin,xmax,ymax]*100*N
        if np_boxes_num[0] <= 0:
            return [[], []]

        # mot_bboxes = mot_result.get('boxes')
        mot_bboxes = mot_result
        ...
        for idx in range(len(mot_bboxes)):
            ...
            action_ret = {'class': 1.0, 'score': -1.0, 'boxes': []}
            box_num = np_boxes_num[idx]
            boxes = det_result['boxes'][cur_box_idx:cur_box_idx + box_num]
            cur_box_idx += box_num
            isvalid = (boxes[:, 1] > self.threshold) & (boxes[:, 0] == 0)
            valid_boxes = boxes[isvalid, :] ## [M,6] but use only first index
            if valid_boxes.shape[0] >= 1:
                action_ret['class'] = valid_boxes[0, 0]
                action_ret['score'] = valid_boxes[0, 1]
                action_ret['boxes'] = valid_boxes[0, 2:]
                self.result_history[
                    tracker_id] = [0.0, self.frame_life, valid_boxes[0, 1], valid_boxes[0,2:]]
            else:
                history_det, life_remain, history_score, history_box = self.result_history.get(
                ...
                if life_remain <= 0 and tracker_id in self.result_history:
                    ...
                    self.result_history[tracker_id] = [
                        history_det, life_remain, history_score, history_box
                    ]
    ...
    def check_id_is_same(self, result):
        # result [N,11] [id,score,xmin,ymin,xmax,ymax,w,h,i,j,k]
        # result [N,7] [xmin,ymin,xmax,ymax,id,score,cls]
        return np.all(np.isin(result[:,7],self.id_in_last_frame))
