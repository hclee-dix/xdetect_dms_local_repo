...
class XFallingDetector(XDetector):
    def __init__(self,request:IDetect,model_path,path,config):
        super().__init__()
    ...
    def init_callback(self):
        self.callback = XFallingDetectorCallback(self.request,self.annotator.labelMap)
    
    def init_source(self):
        self.capture = XFallingDetectorCapture(self.request.source_type,self.path,self.annotator.meta)
    ...
    def predict_stream(self):
        ...
        for frame_count,(frame,im) in enumerate(self.capture):
            ...
            if onCallback and not self.is_fall:
                self.callback.onPredictEpochEnd(annotated_frame,perf,self.result,frame_count)
            ###########
...
def createXFallingDetector(request:IDetect):
    detector = XFallingDetector(
        request=request,
