...
import dev.ai.xFire.net.slowfast.utils.distributed as du
import dev.ai.xFire.net.slowfast.utils.logging as logging
from dev.ai.xFire.net.slowfast.utils.c2_model_loading import get_name_convert_func
from dev.ai.xFire.net.slowfast.utils.env import checkpoint_pathmgr as pathmgr
...
def load_checkpoint(
    ...
    if convert_from_caffe2:
        ...
        with pathmgr.open(path_to_checkpoint, "rb") as f:
            ...
            # checkpoint = torch.load(f, map_location="cuda:0", encoding="latin1")
        ...
        if inflation:
            ...
            logger.info(f"pre_train_dict_match: {type(pre_train_dict_match)}")
            # Weights that do not have match from the pre-trained model.
            not_load_layers = [k for k in model_dict.keys() if k not in pre_train_dict_match.keys()]
            logger.info(f"not_load_layers: {type(not_load_layers)}")
            logger.info(f"not_load_layers: {not_load_layers}")
...
def load_test_checkpoint(cfg, model):
    ...
    if cfg.TEST.CHECKPOINT_FILE_PATH != "":
        ...
        # TEST하려면 checkpoint 파일이 필요한듯, 없을 때 세팅하는 구간이 어딘가 설정되어 있는데 찾아야 함
        # cfg.TEST.CHECKPOINT_FILE_PATH = "data/models/checkpoints/checkpoint_epoch_00001.pyth"
        load_checkpoint(
