...
import dev.ai.xFire.net.slowfast.datasets.ava_helper as ava_helper
import dev.ai.xFire.net.slowfast.utils.logging as logging
import dev.ai.xFire.net.slowfast.utils.metrics as metrics
import dev.ai.xFire.net.slowfast.utils.misc as misc
from dev.ai.xFire.net.slowfast.utils.ava_eval_helper import (
    evaluate_ava,
    read_csv,
    read_exclusions,
    read_labelmap,
...
class TestMeter(object):
    ...
    def update_stats(self, preds, labels, clip_ids):
        ...
        Args:
            preds (tensor): predictions from the current batch. Dimension is
                N x C where N is the batch size and C is the channel size (num_cls).
            labels (tensor): the corresponding labels of the current batch. Dimension is N.
            clip_ids (tensor): clip indexes of the current batch, dimension is N.
        """
        for ind in range(preds.shape[0]):
            # print(f"idx: {ind}, pred: {preds[ind]}, label: {self.video_labels[int(clip_ids[ind]) // self.num_clips]}, clip_ids: {int(clip_ids[ind])}, num_clips: {self.num_clips}")
    ...
    def finalize_metrics(self, ks=(1, 5)):
        ...
        if self.multi_label:
            mean_ap, mtr, acc = get_map(self.video_preds.cpu().numpy(), self.video_labels.cpu().numpy())
            map_str = "{:.{prec}f}".format(mean_ap * 100.0, prec=2)
            self.stats["map"] = map_str
            self.stats["top1_acc"] = map_str
            self.stats["top5_acc"] = map_str
            # self.stats["acc"] = acc
            self.stats["confusion_matrix"] = mtr
        else:
            ...
            logger.info(f"VIDEO LABELS: {self.video_labels}")
            logger.info(f"VIDEO PREDS: {self.video_preds}")
            # acc = accuracy_score(self.video_labels, self.video_preds)
...
def get_map(preds, labels):
    ...
    try:
        ...
        cf_mtr = confusion_matrix(labels, preds)
        # acc = accuracy_score(labels, preds)
    except ValueError:
        print("Average precision requires a sufficient number of samples in a batch which are missing in this sample.")

    mean_ap = np.mean(aps)
    return mean_ap, cf_mtr, acc
