...
from dev.ai.xFire.net.slowfast.utils.env import pathmgr
...
class Kinetics(torch.utils.data.Dataset):
    ...
    def _construct_loader(self):
        ...
        """
        path_to_file = os.path.join(self.cfg.DATA.PATH_TO_DATA_DIR, f"{self.cfg.DATA.FOLDER}_{self.mode}.csv")
        print("************************************")
        logger.info(f"MODE: {self.cfg.DATA.FOLDER}_{self.mode}")
        logger.info(f"PATH TO DATA DIR: {self.cfg.DATA.PATH_TO_DATA_DIR}")
        ...
        assert (len(self._path_to_videos) > 0), (
            "Failed to load Kinetics split {} from {}".format(self._split_idx, path_to_file)
        )
        logger.info(
            "Constructing kinetics dataloader (size: {} skip_rows {}) from {} ".format(
                len(self._path_to_videos) if self.mode != "test" else len(self._path_to_videos)/self._num_clips,
                self.skip_rows, path_to_file
            )
        )
    ...
    def __getitem__(self, index):
        ...
        # logger.info(f"NUM_DECODE: {num_decode}")
        ...
        # logger.info(f"SCALE MIN: {min_scale}, MAX: {max_scale}, CROP: {crop_size}")
        ...
        for i_try in range(self._num_retries):
            ...
            try:
                # logger.info(f"IDX: {index}, PATH: {self._path_to_videos[index]}")
                video_container = container.get_video_container(
                    self._path_to_videos[index],
                    self.cfg.DATA_LOADER.ENABLE_MULTI_THREAD_DECODE,
                    self.cfg.DATA.DECODING_BACKEND,
                )
                # logger.info(f"VIDEO CONTAINER: {type(video_container)}")
            ...
            # logger.info(f"num DECODE: {num_decode}, frames: {len(num_frames)}")
