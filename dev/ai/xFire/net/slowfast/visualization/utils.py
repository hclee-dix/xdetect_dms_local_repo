...
from dev.ai.xFire.net.slowfast.datasets.utils import pack_pathway_output, tensor_normalize
...
def process_cv2_inputs(frames, cfg):
    ...
    """
    inputs = torch.from_numpy(np.array(frames)).float() / 255 # 0.040
    inputs = tensor_normalize(inputs, cfg.DATA.MEAN, cfg.DATA.STD) # 0.08
    # T H W C -> C T H W.
    inputs = inputs.permute(3, 0, 1, 2) # 0.000024
    # Sample frames for num_frames specified.
    index = torch.linspace(0, inputs.shape[1] - 1, cfg.DATA.NUM_FRAMES).long() #0.000050
    inputs = torch.index_select(inputs, 1, index) # 0.049
    inputs = pack_pathway_output(cfg, inputs) #0.003
    inputs = [inp.unsqueeze(0) for inp in inputs] #0.00003
    return inputs
