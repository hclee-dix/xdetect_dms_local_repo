...
from dev.ai.xBase.label import FIRE
#
...
#
from dev.util.ops import getAvgResultFromTotalFrame, getTopResultFromSingleFrame

class XFireDetectorCallback(XCallback):
    ...
    def onPredictEnd(self, imageList, perfList, requestList):
        h,w,c = imageList[0].shape
        ##
        src_path, dst_path = makeAsset('data_out', imageList, {'project_id': self.request.project_id, 'd_base_model_id': self.request.d_detect_model_id, 'ext': self.ext, 'fps': self.fps}, None, {'history_id': self.request.d_detect_history_id, 'format': ""})
        storage_url = uploadFile(src_path, dst_path)
        ##
        result, accuracy, perf = getAvgResultFromTotalFrame(perfList, requestList)
        history = IDetectHistory(
            ...
            result_list=[IDetectHistoryResult(index=int(data[5]),conf=data[4],name=self.labelMap[int(data[5])]) for data in result]
        )
        print(history)
    ...
    def onPredictEpochEnd(self, image, perf, result, frame_num = None):
        h,w,c = image.shape
        ##
        hid = self.request.d_detect_history_id if not frame_num else f"{self.request.d_detect_model_id}_{frame_num}"
        ...
        storage_url = uploadFile(src_path, dst_path)
        ##
        topResult, accuracy = getTopResultFromSingleFrame(result)
        history = IDetectHistory(
            org_id=self.request.organization_id,
            project_id=self.request.project_id,
            d_detect_model_id=self.request.d_detect_model_id,
            ...
            fps=10000/perf,
            accuracy=float(accuracy),
            result_list=[IDetectHistoryResult(index=int(data[5]), conf=data[4], name=self.labelMap[int(data[5])]) for data in topResult]
        )
        print(history)
        updateDetectHistory(history, hid)
        return topResult

class XFireTrainerCall(XCallback):
    def __init__(self, request):
        self.request:ITrain = request
        super().__init__()
    
    def onTrainStart(self):
        print("train start")

    def onTrainEpochStart(self):
        print('epoch start')
        
    def onTrainEpochEnd(self):
        print('epoch end')
    
    def onValidateStart(self):
        print('validate end')
...
class XTestCallback(XCallback):
    ...
    def onPredictEpochEnd(self, image, perf, result, frame_num=None):
        ...
        src_path, dst_path = makeAsset('data_out', image, {'project_id':'PID', 'detect_model_id': 'HID', 'ext': 'jpg', 'fps': 0}, 'None', {'history_id': 'HID'+f'_{frame_num}' if frame_num else 'HID', "format": ""}, True)
        topResult, accuracy = getTopResultFromSingleFrame(result)
        history = IDetectHistory(
            org_id='OID',
            project_id='PID',
            ...
            storage_url='',
            source_type='image',
            resolution=IDetectHistoryResolution(width=w, hieght=h),
            inference=perf/1000,
            fps=1000/perf,
            accuracy=float(accuracy),
            result_list=[IDetectHistoryResult(index=int(data[5]),conf=data[4],name=self.labelMap[int(data[5])]) for data in topResult]
