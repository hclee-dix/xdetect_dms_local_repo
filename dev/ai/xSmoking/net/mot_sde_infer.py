...
from dev.ai.xSmoking.net.preprocess import decode_image
...
from dev.ai.xSmoking.net.detector import Detector, PredictConfig, load_predictor
from dev.ai.xSmoking.net.mot_utils import argsparser, Timer, get_current_memory_mb, video2frames, _is_valid_video
from dev.ai.xSmoking.net.mot.tracker import JDETracker, DeepSORTTracker, OCSORTTracker, BOTSORTTracker
from dev.ai.xSmoking.net.mot.utils import MOTTimer, write_mot_results, get_crops, clip_box, flow_statistic
from dev.ai.xSmoking.net.mot.visualize import plot_tracking, plot_tracking_dict

from dev.ai.xSmoking.net.mot.mtmct.utils import parse_bias
from dev.ai.xSmoking.net.mot.mtmct.postprocess import trajectory_fusion, sub_cluster, gen_res, print_mtmct_result
from dev.ai.xSmoking.net.mot.mtmct.postprocess import get_mtmct_matching_results, save_mtmct_crops, save_mtmct_vis_results
...
class SDE_Detector(Detector):
    ...
    def tracking(self, det_results, img=None):
        ...
        if self.use_deepsort_tracker:
            ...
            online_scores = defaultdict(list)
            online_ids = defaultdict(list)
            online_whs = defaultdict(list)
            online_smoking = defaultdict(list)
            for t in online_targets:
                tlwh = t.tlwh
                tid = t.track_id
                tscore = t.score
                if tlwh[2] * tlwh[3] <= self.tracker.min_box_area:
                    continue
                online_tlwhs[0].append(tlwh)
                online_whs[0].append(tlwh[2:])
                online_ids[0].append(tid)
                online_scores[0].append(tscore)
                online_smoking[0].append(np.zeros(3))

            tracking_outs = {
                'online_tlwhs': online_tlwhs,
                'online_scores': online_scores,
                'online_ids': online_ids,
                'online_whs': online_whs,
                'online_smoking': online_smoking,
            }
            r1 = np.array(tracking_outs['online_ids'][0]).reshape(-1,1)
            r2 = np.array(tracking_outs['online_scores'][0]).reshape(-1,1)
            r3 = np.array(tracking_outs['online_tlwhs'][0]).reshape(-1,4)
            r4 = np.array(tracking_outs['online_whs'][0]).reshape(-1,2)
            r5 = np.array(tracking_outs['online_smoking'][0]).reshape(-1,3)
            return np.concatenate((r1,r2,r3,r4,r5),axis=1)
    ...
    def predict_image(self,
        ...
        for frame_id, img_file in enumerate(image_list):
            ...
            if self.do_mtmct:
                ...
                mot_results.append(tracking_outs)
