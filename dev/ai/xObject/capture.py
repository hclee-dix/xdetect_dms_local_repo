...
class XObjectDetectorCapture(XCapture):
    ...
    def init_source(self,source_type,path):
        if source_type == 'image':
            ...
            self.meta.width = self.image.shape[1]
            self.meta.height = self.image.shape[0]
            self.meta.fps = 0
    ...
    def __next__(self):
        self.frame = self.retrieve() if self.source_type != 'image' else self.image
