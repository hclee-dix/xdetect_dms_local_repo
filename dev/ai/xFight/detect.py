...
class XFightDetector(XDetector):
    ...
    def init_callback(self):
        self.callback = XFightDetectorCallback(self.request,self.annotator.labelMap)

    def init_source(self):
        self.capture = XFightDetectorCapture(self.request.source_type,self.path,self.annotator.meta)
    ...
    def initFightDetector(self):
        ...
        if self.with_video_action:
            ...
            self.sample_freq = self.cfg["VIDEO_ACTION"]["sample_freq"] ## sampling intervals 
...
def createXFightDetector(request:IDetect):
    detector = XFightDetector(
        request=request,
